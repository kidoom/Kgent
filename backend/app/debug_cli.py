...
from app.agent.messages import AgentStep, Message, ToolResultBlock, ToolUseBlock
from app.agent.model_client import ModelClientError, ModelClientProtocol, available_providers, build_model_client
from app.agent.session_store import get_or_create_session, reset_sessions
...
Examples:
  ...
  python -m app.debug_cli --once "帮我算一下 12 * 8 + 6"
  python -m app.debug_cli --provider heuristic
  python -m app.debug_cli --compact --once "帮我算一下 12 * 8 + 6"
        """.strip(),
    )
    parser.add_argument(
        "--compact",
...
async def _run_interactive(args: argparse.Namespace) -> None:
    ...
    model_client, tools, step_limit, settings, provider, tracer = context
    _print_startup_config(settings, provider, step_limit, args.session_id)
...
async def _run_single_shot(args: argparse.Namespace, user_input: str) -> None:
    ...
    print("\nTip: run `python -m app.debug_cli` with no arguments for sustained interactive mode.")


async def _build_runtime(
    args: argparse.Namespace,
) -> tuple[ModelClientProtocol, list, int, Any, str, Any] | None:
    reload_dotenv_settings()
    ...
    return model_client, tools, step_limit, settings, provider, tracer


def _print_startup_config(settings: Any, provider: str, step_limit: int, session_id: str) -> None:
    print("\n=== Configuration ===")
...
def print_agent_step(step: AgentStep, indent: str = "") -> None:
    ...
    if step.type == "think":
        ...
        print(f"{prefix}[call] turn={step.turn_index} tool={step.tool_name} id={step.tool_use_id}")
